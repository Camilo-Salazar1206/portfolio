export enum RoutesEnum {
  Home = 'home',
  About = 'about me',
  Skills = 'skills',
  Work = 'experience',
  Proyects = 'proyects',
  Contact = 'contact',
}

export enum RouteTranslations {
  Home = 'Inicio',
  About = 'Acerca de mí',
  Proyects = 'Proyectos',
  Skills = 'Habilidades',
  Work = 'Experiencia',
  Contact = 'Contacto',
}

export enum Links {
  GITHUB = 'https://github.com/Camilo-Salazar1206',
  LINKEDIN = 'https://www.linkedin.com/in/camilo-salazar1206/',
  GOOGLE_DRIVE = 'https://drive.google.com/file/d/1ksZTXPpR1k1LdJ9NR3dcSwvfse93bJ2P/view?usp=sharing',
  EMAIL = 'mailto:c4m1loo12@gmail.com',
  INSTAGRAM = 'https://www.instagram.com/ts_camilo06/'
}


export enum SkillsStack {
  Angular = 'Angular',
  Html = 'HTML',
  Css = 'CSS',
  Scss = 'SCSS',
  TypeScript = 'TypeScript',
  JavaScript = 'JavaScript',
  NodeJS = 'NodeJS',
  Express = 'Express',
  MySQL = 'MySQL',
  Github = 'Github',
  vsCode = 'Visual Studio Code',
  MongoDB = 'MongoDB',
  Aws = 'AWS',
  Java = 'Java',
  Springboot = 'Springboot',
  Kotlin = 'Kotlin',
  Jetpack = 'Jetpack Compose',
  React = 'React',
  reactNative = 'React Native',
  Docker="Docker"
}

export enum iconDirection {
  Angular = 'https://img.icons8.com/fluent/512/angularjs.png',
  TypeScript = 'https://cdn-icons-png.flaticon.com/512/5968/5968381.png',
  JavaScript = 'https://cdn-icons-png.flaticon.com/128/5968/5968292.png',
  Html = 'https://cdn-icons-png.flaticon.com/512/732/732212.png',
  Css = 'https://cdn-icons-png.flaticon.com/128/732/732190.png',
  Scss = 'https://cdn-icons-png.flaticon.com/512/5968/5968358.png',
  NodeJS = 'https://static-00.iconduck.com/assets.00/node-js-icon-909x1024-ig4gm9k7.png',
  Express = 'https://img.icons8.com/color/512/express-js.png',
  MySQL = 'https://upload.wikimedia.org/wikipedia/labs/8/8e/Mysql_logo.png',
  Github = 'https://static-00.iconduck.com/assets.00/git-icon-2048x2048-juzdf1l5.png',
  vsCode = 'https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/2048px-Visual_Studio_Code_1.35_icon.svg.png',
  MongoDB = 'https://www.svgrepo.com/show/331488/mongodb.svg',
  Aws = 'https://cdn.iconscout.com/icon/free/png-256/free-aws-logo-icon-download-in-svg-png-gif-file-formats--cloud-computing-network-server-database-brand-pack-logos-icons-1583149.png?f=webp&w=256',
  Java = 'https://cdn.freebiesupply.com/logos/large/2x/java-logo-png-transparent.png',
  Springboot = 'https://img.icons8.com/?size=512&id=90519&format=png',
  Kotlin = 'https://upload.wikimedia.org/wikipedia/commons/thumb/7/74/Kotlin_Icon.png/1200px-Kotlin_Icon.png',
  jetpack = 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjC97Z8BResg5dlPqczsRCFhP6zewWX0X0e7fVPG-G7PuUZwwZVsi9OPoqJYkgqT2h0FI95SsmWzVEgpt8b8HAqFiIxZ98TFtY4lE0b8UrtVJ2HrJebRwl6C9DslsQDl9KnBIrdHS6LtkY/s1600/jetpack+compose+icon_RGB.png',
  reactNative = 'https://cdn4.iconfinder.com/data/icons/logos-3/600/React.js_logo-512.png',
  Docker="https://cdn4.iconfinder.com/data/icons/logos-and-brands/512/97_Docker_logo_logos-512.png"
}
export enum documentationDirection {
  Angular = 'https://angular.io/docs',
  Html = 'https://developer.mozilla.org/en-US/docs/Web/HTML',
  Css = 'https://developer.mozilla.org/en-US/docs/Web/CSS',
  Scss = 'https://sass-lang.com/documentation',
  TypeScript = 'https://www.typescriptlang.org/docs/',
  JavaScript = 'https://developer.mozilla.org/en-US/docs/Web/JavaScript',
  NodeJS = 'https://nodejs.org/en/docs/',
  Express = 'https://expressjs.com/en/guide.html',
  MySQL = 'https://dev.mysql.com/doc/',
  Github = 'https://docs.github.com/en',
  vsCode = 'https://code.visualstudio.com/docs',
  MongoDB = 'https://www.mongodb.com/docs/manual/',
  Aws = 'https://docs.aws.amazon.com/',
  Java = 'https://docs.oracle.com/en/java/',
  Springboot = 'https://docs.spring.io/spring-boot/docs/current/reference/htmlsingle/',
  Kotlin = 'https://kotlinlang.org/docs/home.html',
  Jetpack = 'https://developer.android.com/jetpack/compose/documentation',
  reactNative = 'https://reactnative.dev/docs/getting-started',
  React="https://react.dev/learn/build-a-react-app-from-scratch",
  Docker="https://docs.docker.com/"
}

export enum TitleProyect {
  RutasParaMoteros = 'RPM - Rutas Para Moteros',
}

export enum DescriptionProyect {
  DescriptionRutasParaMoteros = 'Aplicación web-móvil desarrollada durante mi formación en el SENA, orientada a motoviajeros en Colombia. Como desarrollador móvil, trabajé en la implementación de rutas personalizadas, carga de imágenes con Firebase, autenticación con JWT y consumo de API REST. La app promueve el moto-turismo responsable, permitiendo a usuarios trazar, compartir y calificar rutas.',
}
export enum ImgUrlProyect {
  ImgRutasParaMoteros = 'https://i.imgur.com/KFu0l3Z.png',
}

export enum UrlProyects {
  RutasParaMoteros = 'https://rutasparamoteros.vercel.app/',
}

export enum GithubUrlProyects {
  RutasParaMoteros_Backend = 'https://github.com/NarvaezSKY/RPMBackEnd',


}

export enum urlButtons {
  urlLinkedin = 'https://www.linkedin.com/in/camilo-salazar1206/',
}

export enum JobTitle {
  FRONTEND_DEVELOPER = 'Frontend Developer',
}

export enum JobTecnologies{
  FRONTEND_DEVELOPER_PRAGMA="Angular / TypeScript / Jasmine / SCSS"
}
export enum JobCompany {
  PRAGMA = 'PRAGMA',
}

export enum JobDescription {
  FRONTEND_DEVELOPER = `Entre abril y octubre de 2024, desempeñé el rol de Frontend Developer, participando activamente en el diseño y desarrollo de interfaces web modernas utilizando Angular y TypeScript.
Implementé diseños responsivos basados en buenas prácticas de UX/UI, integré servicios RESTful mediante HTTPClient, y optimicé el rendimiento de las aplicaciones.

Utilicé Jasmine y Karma para realizar pruebas unitarias, asegurando la estabilidad del código.
Además, colaboré estrechamente con diseñadores UX/UI, gestioné el control de versiones con Git y seguí metodologías ágiles como Scrum mediante reuniones diarias, planificación de sprints y revisiones continuas del producto.

Me enfoqué en escribir código limpio, reutilizable y mantenible, garantizando la calidad y escalabilidad de los proyectos.`
}


export enum JobDateExperience {
  DATE_PRAGMA = 'Abril 2024 - Octubre 2024',
}

export enum NumbersEnum {
  ScrollThreshold = 50,
  TouchMoveThreshold = 100,
  MenuOffset = 80,
  ScrollDuration = 800,
  ScrollSkills=300

}
